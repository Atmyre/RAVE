...
class PromptLearner(nn.Module):
    def __init__(self, config, model, initials=None):
        ...
        if isinstance(initials, list):
            ...
            self.prompt_embedding=torch.nn.init.xavier_normal_(nn.Parameter(model.token_embedding([" ".join(["X"]*config.length_prompt)," ".join(["X"]*config.length_prompt)]).requires_grad_())).cuda()
